      ...
      try {
        ...
        if (artistResponse.ok) {
          ...
          if ("error" in artistData) {
            // Silently handle error - Spotify API may not be configured
          } else {
            setArtist(artistData);
          }
        }
        // Silently handle non-ok responses - Spotify API may not be configured

        // Handle albums data
        if (albumsResponse.ok) {
          const albumsData = await albumsResponse.json();
          // Check if response contains an error object
          if (!("error" in albumsData) && Array.isArray(albumsData)) {
        ...
        // Silently handle network errors - Spotify API may not be configured
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <main className="mx-auto max-w-7xl animate-fade-in px-6 pt-32 pb-12 md:px-12">
        {isLoading ? (
          ...
          <>
            ...
            {/* Empty State */}
            {!artist && albums.length === 0 && !isLoading && (
